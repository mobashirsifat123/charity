...
export default function AdminCategoriesManager() {
  ...
  async function loadCategories() {
    try {
      setLoading(true);
      const result = await adminFetchJson("/api/admin/resources/article-categories");
      setItems(result.data || []);
    } catch (error) {
      setMessage({ type: "danger", text: error.message || "Unable to load categories." });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (isEditing) {
        await adminFetchJson(`/api/admin/resources/article-categories/${form.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ payload: { name, slug } }),
        });
      ...
      setMessage({ type: "danger", text: error.message || "Unable to save category." });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id) {
    ...
    try {
      await adminFetchJson(`/api/admin/resources/article-categories/${id}`, { method: "DELETE" });
      setItems((prev) => prev.filter((item) => item.id !== id));
      if (form.id === id) resetForm();
      setMessage({ type: "success", text: "Category deleted." });
    } catch (error) {
      setMessage({ type: "danger", text: error.message || "Unable to delete category." });
    }
  }
    ...
    <div className="d-flex flex-column gap-4">
      <div className="card border-0 shadow-sm rounded-4">
        <div className="card-body p-4">
          ...
          <p className="text-muted mb-0">Manage taxonomy used in article filters and content organization.</p>
        </div>
      </div>

      <div className="card border-0 shadow-sm rounded-4">
        <div className="card-body p-4">
          <h5 className="fw-bold mb-3">{isEditing ? "Edit Category" : "Create Category"}</h5>
            ...
            <div className="table-responsive">
              <table className="table mb-0 align-middle">
                ...
                <tbody>
                  ...
                  {!sortedItems.length ? (
                    <tr>
                      <td colSpan={3} className="text-center text-muted py-4">No categories found yet.</td>
                    </tr>
                  ) : null}
                </tbody>
              </table>
