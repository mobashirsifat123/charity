...
function AdminFatwasContent() {
  ...
  const handleToggleStatus = async (fatwa) => {
    ...
    try {
      ...
      setFatwas(
        fatwas.map((f) =>
          f.id === fatwa.id ? { ...f, status: newStatus } : f,
        ),
      );
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm(
        "Are you absolutely sure you want to delete this Fatwa ruling?",
      )
    )
      return;
    try {
      await adminFetchJson(`/api/admin/resources/fatwas/${id}`, {
        method: "DELETE",
      });
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredAndSortedFatwas.length / itemsPerPage),
  );
  const currentFatwas = filteredAndSortedFatwas.slice(
        ...
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
          <div>
            ...
            <p className="text-muted mb-0">
              Manage community questions and publish official Islamic rulings.
            </p>
          </div>
          <div>
            <Link
            ...
            <div className="row g-3 align-items-center">
              ...
              <div className="col-lg-4">
                <div className="d-flex gap-2">
                  <select
                    className="form-select bg-light focus-ring-none"
                    value={statusFilter}
                    onChange={(e) => {
              ...
              {selectedIds.size > 0 && (
                <div className="col-lg-3 text-lg-end">
                  ...
                  >
                    <i className="fa-solid fa-trash me-2"></i> Drop{" "}
            ...
            {loading ? (
              ...
              <div className="text-center py-5">
                ...
                <p className="text-muted">
                  No fatwas matched your query. Clear filters or add a new
                  record.
                </p>
              </div>
            ) : (
                    ...
                    <tr>
                      <th className="px-4 py-3" style={{ width: "50px" }}>
                        <input
                          ...
                          checked={
                            selectedIds.size > 0 &&
                            selectedIds.size === currentFatwas.length
                          }
                      ...
                      >
                        ...
                        <td className="py-3">
                          <div className="d-flex align-items-center gap-3">
                            <div
                              className="bg-light rounded-circle text-center d-flex align-items-center justify-content-center border"
                            ...
                            <div>
                              <div className="fw-bold text-dark">
                                {fatwa.title?.slice(0, 60)}
                                {fatwa.title?.length > 60 && "..."}
                              </div>
                              <div className="text-muted small font-monospace">
                                UUID: {fatwa.id.split("-")[0]}
                              </div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3 text-end">
                          <div className="d-flex gap-2 justify-content-end">
                            <Link
                              href={`/admin/fatwas/edit/${fatwa.id}`}
              ...
              <div className="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                <span className="text-muted small">
                  ...
                  <span className="fw-bold text-dark">
                    {Math.min(
                      currentPage * itemsPerPage,
                      filteredAndSortedFatwas.length,
                    )}
                  </span>{" "}
                  records of{" "}
                  <span className="fw-bold text-dark">
