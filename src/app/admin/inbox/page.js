...
const REQUEST_STATUSES = ['all', 'new', 'reviewing', 'answered', 'archived', 'converted'];
const SUBSCRIBER_STATUSES = ['all', 'active', 'paused', 'unsubscribed', 'archived'];
...
const getFriendlyError = (message = '') => {
  ...
  if (lower.includes('does not exist') || lower.includes('relation') || lower.includes('schema cache')) {
...
export default function AdminInboxPage() {
  ...
  const fetchInbox = useCallback(async () => {
    try {
      ...
      const [requestResult, subscriberResult] = await Promise.all([
        adminFetchJson('/api/admin/resources/fatwa-requests'),
        adminFetchJson('/api/admin/resources/newsletter'),
      ]);
  ...
  const filteredRequests = useMemo(() => {
    ...
    return requests.filter((item) => {
      ...
      const matchesSearch = !lower || [item.name, item.email, item.category, item.question, item.details]
        .filter(Boolean)
  ...
  const filteredSubscribers = useMemo(() => {
    ...
    return subscribers.filter((item) => {
      const matchesStatus = subscriberStatusFilter === 'all' || item.status === subscriberStatusFilter;
      const matchesSearch = !lower || [item.email, item.source, item.status]
        .filter(Boolean)
        .some((value) => String(value).toLowerCase().includes(lower));
  ...
  const convertRequestToFatwa = async (requestItem) => {
    ...
    try {
      ...
      const result = await adminFetchJson(`/api/admin/fatwa-requests/${requestItem.id}/convert`, {
        method: 'POST',
      });

      setRequests((prev) =>
        prev.map((item) => (item.id === requestItem.id ? { ...item, status: 'converted' } : item))
      );
  ...
  const updateSubscriberStatus = async (subscriberId, status) => {
    try {
      ...
      setSubscribers((prev) => prev.map((item) => (item.id === subscriberId ? { ...item, status } : item)));
  ...
  const exportRequests = () => {
    downloadCsv('irwa-fatwa-requests.csv', [
      ['ID', 'Date', 'Name', 'Email', 'Category', 'Status', 'Question', 'Details'],
    ...
    <section className="bg-light pb-5 min-vh-100 position-relative">
      ...
      <div className="container py-4">
        <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center mb-4 gap-3">
          <div>
            <h2 className="fw-bold mb-1"><i className="fa-regular fa-envelope-open text-primary me-2"></i>Admin Inbox</h2>
            <p className="text-muted mb-0">Review incoming fatwa requests, grow your audience, and turn good submissions into publishable drafts.</p>
          </div>
          <div className="d-flex flex-wrap gap-2">
            <Link href="/admin/fatwas" className="btn btn-outline-primary rounded-pill px-4">
              Manage Fatwas
            </Link>
            <button type="button" className="btn btn-outline-secondary rounded-pill px-4" onClick={fetchInbox}>
              Refresh
            </button>
          </div>
        </div>
            ...
            <div className="card border-0 shadow-sm rounded-4 h-100">
              <div className="card-body d-flex align-items-center">
                ...
                <div>
                  ...
                  <h3 className="fw-bold mb-0">{requestCounts.answered + requestCounts.converted}</h3>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="d-flex flex-column flex-lg-row gap-2">
                <div className="input-group">
                  ...
                  <input
                    ...
                    placeholder={activeTab === 'requests' ? 'Search requests by name, email, or question...' : 'Search subscribers by email or source...'}
                    value={searchTerm}
                ...
                {activeTab === 'requests' ? (
                  <>
                    ...
                    >
                      {REQUEST_STATUSES.map((status) => (
                        <option key={status} value={status}>{status === 'all' ? 'All Statuses' : humanizeStatus(status)}</option>
                      ))}
                    </select>
                    <button type="button" className="btn btn-outline-success" onClick={exportRequests} disabled={!filteredRequests.length}>
                  ...
                  <>
                    ...
                    <button type="button" className="btn btn-outline-success" onClick={exportSubscribers} disabled={!filteredSubscribers.length}>
                      <i className="fa-solid fa-file-csv me-2"></i>Export
                    </button>
                  </>
                )}
        ...
        {loading ? (
          ...
          <div className="card border-0 shadow-sm rounded-4 overflow-hidden">
            <div className="card-header bg-white border-bottom p-4">
              ...
              <p className="text-muted mb-0">{filteredRequests.length} request(s) match the current view.</p>
            </div>
            <div className="card-body p-0">
              {filteredRequests.length === 0 ? (
                <div className="text-center py-5">
                  ...
                  <p className="text-muted mb-0">Try a different search or status filter.</p>
                </div>
              ) : (
                    ...
                    <tbody>
                      {filteredRequests.map((item) => (
                        <Fragment key={item.id}>
                          <tr key={item.id}>
                            <td className="text-muted fw-medium">{formatDate(item.created_at)}</td>
                            <td>
                              <div className="fw-bold">{item.name}</div>
                              <a href={`mailto:${item.email}`} className="text-decoration-none small">{item.email}</a>
                            </td>
                            ...
                            <td style={{ minWidth: '170px' }}>
                              <select
                                ...
                                value={item.status}
                                onChange={(event) => updateRequestStatus(item.id, event.target.value)}
                                disabled={processingKey === `request-status-${item.id}`}
                              >
                                {REQUEST_STATUSES.filter((status) => status !== 'all').map((status) => (
                            ...
                            <td className="text-end">
                              <div className="d-flex flex-wrap gap-2 justify-content-end">
                                ...
                                >
                                  {expandedRequestId === item.id ? 'Hide' : 'View'}
                                </button>
                                <button
                                  type="button"
                                  className="btn btn-sm btn-outline-primary"
                                  onClick={() => convertRequestToFatwa(item)}
                                  disabled={processingKey === `request-convert-${item.id}` || item.status === 'converted'}
                                >
                                  {processingKey === `request-convert-${item.id}` ? 'Converting...' : 'Create Draft'}
                                </button>
                                <button
                                  type="button"
                                  className="btn btn-sm btn-outline-danger"
                                  onClick={() => deleteRequest(item.id)}
                                  disabled={processingKey === `request-delete-${item.id}`}
                                >
                                  Delete
                                </button>
                              </div>
                          ...
                          {expandedRequestId === item.id ? (
                            <tr className="bg-light">
                              <td colSpan="5">
                                <div className="p-3">
                                  <div className="mb-3">
                                    <span className="badge bg-secondary-subtle text-dark border">{humanizeStatus(item.status)}</span>
                                  </div>
                                  <div className="row g-3">
                                    <div className="col-lg-6">
                                      <h6 className="fw-bold">Full Question</h6>
                                      <p className="mb-0 text-muted">{item.question}</p>
                                    </div>
                                    <div className="col-lg-6">
                                      <h6 className="fw-bold">Additional Details</h6>
                                      <p className="mb-0 text-muted">{item.details || 'No extra details provided.'}</p>
                                    </div>
                                  </div>
                                </div>
                              </td>
          ...
          <div className="card border-0 shadow-sm rounded-4 overflow-hidden">
            <div className="card-header bg-white border-bottom p-4">
              ...
              <p className="text-muted mb-0">{filteredSubscribers.length} subscriber(s) match the current view.</p>
            </div>
            <div className="card-body p-0">
                    ...
                    <tbody>
                      {filteredSubscribers.map((item) => (
                        <tr key={item.id}>
                          <td className="text-muted fw-medium">{formatDate(item.created_at)}</td>
                          <td>
                            <a href={`mailto:${item.email}`} className="fw-semibold text-decoration-none">{item.email}</a>
                          </td>
                          <td className="text-muted">{item.source || 'website'}</td>
                          <td style={{ minWidth: '180px' }}>
                            <select
                              className="form-select form-select-sm bg-light"
                              value={item.status}
                              onChange={(event) => updateSubscriberStatus(item.id, event.target.value)}
                              disabled={processingKey === `subscriber-status-${item.id}`}
                            >
                              {SUBSCRIBER_STATUSES.filter((status) => status !== 'all').map((status) => (
                                <option key={status} value={status}>{humanizeStatus(status)}</option>
                              ))}
                            </select>
                          </td>
                          <td className="text-end">
                            <button
                              ...
                              disabled={processingKey === `subscriber-delete-${item.id}`}
                            >
                              Delete
                            </button>
                          </td>
