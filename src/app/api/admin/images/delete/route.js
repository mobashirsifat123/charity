...
function revalidateImageTarget(tableName, rowId) {
  ...
  if (tableName === "blogs") {
    revalidatePath("/");
    revalidatePath("/blog-grid");
    if (rowId) revalidatePath(`/blog-details/${rowId}`);
  }

  if (tableName === "fatwas") {
    revalidatePath("/");
    revalidatePath("/fatwa");
    if (rowId) revalidatePath(`/fatwa/${rowId}`);
  }
  ...
  if (tableName === "campaigns") {
    ...
    if (rowId) revalidatePath(`/cause-details/${rowId}`);
  }
}
...
export async function POST(req) {
  try {
    ...
    const { supabase } = auth;
    const { url, image_url, table_name, column_name, row_id, setting_key } =
      await req.json();
    ...
    revalidateImageTarget(resolvedTableName, row_id);
